...
# The host is set to 'mysql-container' as defined in the Docker Compose file
DBhost = os.getenv('MYSQL_HOST', 'mysql')  # Use 'mysql' as the hostname
DBuser = os.getenv('DB_USER', 'flask_user')       # Use the username defined in Docker
DBpassword = os.getenv('DB_PASSWORD', 'flask_password')  # Use the password defined in Docker
DBname = os.getenv('DB_NAME', 'flask_db')         # Use the database name defined in Docker
...
try:
    ...
    # You can add your SQL queries here using the cursor object
...
class User:
	...
	def update_info(self,First_Name,Last_Name,NewPhno,OldPhno):
		cnx=mysql.connector.connect(host=DBhost,user=DBuser,password=DBpassword,database=DBname)
		cursor=cnx.cursor()
		#using phone no a unique value to update new value
...
class Appointment:
	...
	def aptmnt_doctors(self,identifier,spec,Gender,order):
		cnx=mysql.connector.connect(host=DBhost,user=DBuser,password=DBpassword,database=DBname)
		#This (cursor=cnx.cursor())line creates a cursor object from that connection, enabling you to execute SQL queries and handle results.
		cursor=cnx.cursor()
		if identifier==0:
